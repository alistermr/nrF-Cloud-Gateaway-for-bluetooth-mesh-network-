...
void model_handler_toggle_led(int idx)
{
	if (idx < 0 || idx >= ARRAY_SIZE(led_ctx)) {
		return;
	}

	struct led_ctx *led = &led_ctx[idx];

	/* Cancel any ongoing transition and clear remaining time */
